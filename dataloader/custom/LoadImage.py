...
class UnifiedITKReader(NumpyReader):
    ...
    def read(self, data: Union[Sequence[PathLike], PathLike], **kwargs):
        ...
        for name in filenames:
            ...
            if len(_obj.shape) == 2: # (H,W) -> (3,H,W)
                _obj = np.repeat(np.expand_dims(_obj, axis=0), 3, axis=0)
            elif len(_obj.shape) == 3 and _obj.shape[0] > 3: # (D,H,W) -> (3,H,W)
                _obj = _obj[:3,:, :]
            elif len(_obj.shape) == 3 and _obj.shape[0] == 1: # (1,H,W) -> (3,H,W)
                _obj = np.repeat(_obj, 3, axis=0)
            elif len(_obj.shape) == 3 and _obj.shape[0] == 2: # Greylevel (2,H,W) -> (3,H,W)
                _obj = np.concatenate([_obj, _obj[0:1, :, :]], axis=0)
            else:
                pass
